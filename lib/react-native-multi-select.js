...
export default class MultiSelect extends Component {
  static propTypes = {
    ...
    customAddItemRow: PropTypes.func,
    customEmptyItem: PropTypes.any,
  ...
  _getSelectLabel = () => {
    ...
    return typeof selectText === 'function' ? selectText(selectedItems.length) : `${selectText} (${selectedItems.length} selected)`;
  };
  ...
  _addItem = () => {
    ...
    if (newItemName) {
      const newItemId = newItemName
        ...
        .join('-');
      newItems = [{ [uniqueKey]: newItemId, name: newItemName }, ...items ];
  ...
  _getRowNew = item => {
    const { customAddItemRow} = this.props;
    return(
      <TouchableOpacity
      disabled={item.disabled}
    ...
    >
      {
        customAddItemRow ? customAddItemRow(item.name) : 
        <View>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text
            style={[
  ...
  _filterItems = searchTerm => {
    switch (this.props.filterMethod) {
      case 'full':
        ...
        case 'search' : 
        return this._searchItem(searchTerm);
      default:
  ...
  _filterItemsPartial = searchTerm => {
    ...
    const filteredItems = [];
    items.forEach(item => {
      const parts = searchTerm.toLowerCase().trim().split(/[ \-:]+/);
  ...
  _searchItem = searchTerm => {
    const { items, displayKey } = this.props;
    const filteredItems = [];
    items.forEach(item => {
      if(item[displayKey].toLowerCase().includes(searchTerm.toLowerCase()))
      {
        filteredItems.push(item);
      }
    });
  ...
  _renderItems = () => {
    ...
    if (renderItems.length) {
      itemList = (
        <FlatList
          data={maxItemCount ? renderItems?.slice(0,maxItemCount) : renderItems}
      ...
      searchTermMatch = renderItems.filter(item => item.name.toLowerCase() === searchTerm.toLowerCase())
        .length;
    } else if (!canAddItems) {
      itemList = customEmptyItem ? 
      (typeof customEmptyItem) === 'function' ? customEmptyItem(searchTerm) 
      : customEmptyItem  :
       (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text
          ...
          >
           { typeof customEmptyItem === 'string'? customEmptyItem :  "No item to display."}
          </Text>
        </View>
      );
    }
